...
- [ ] Verify CounselorCategory model exists (AC: 1)
  - [ ] Check `app/models/counselor_category.py` from architecture
  - [ ] Verify model has fields: id, name, description, icon_name, enabled, created_at, updated_at
  - [ ] If not exists, create SQLAlchemy model
  - [ ] Create Alembic migration for counselor_categories table
  
- [ ] Seed database with six categories (AC: 2, 3)
  - [ ] Create seed data script or migration
  - [ ] Define Health category with description
  - [ ] Define Career category with description
  - [ ] Define Academic category with description
  - [ ] Define Financial category with description
  - [ ] Define Social category with description
  - [ ] Define Personal Development category with description
  - [ ] Assign appropriate icon names for each
  - [ ] Set all categories to enabled=True
  
- [ ] Create counselor repository (AC: 4)
  - [ ] Create `app/repositories/counselor_repository.py`
  - [ ] Implement `get_enabled_categories()` method
  - [ ] Use async SQLAlchemy query
  - [ ] Order categories by name alphabetically
  - [ ] Handle database errors gracefully
  
- [ ] Create counselor router and endpoint (AC: 4, 5)
  - [ ] Create `app/routers/counselors.py`
  - [ ] Implement GET /categories endpoint
  - [ ] Require authentication using get_current_user dependency
  - [ ] Call counselor repository to fetch categories
  - [ ] Return JSON response with category list
  - [ ] Register router in main.py with `/api/counselors` prefix
  
- [ ] Create response schemas (AC: 4)
  - [ ] Create `app/schemas/counselor.py`
  - [ ] Define CounselorCategoryResponse Pydantic model
  - [ ] Include fields: id, name, description, icon_name
  - [ ] Exclude enabled and timestamp fields from response
  - [ ] Use Pydantic Config for ORM mode
  
- [ ] Write comprehensive tests (AC: 6)
  - [ ] Test get_enabled_categories repository method
  - [ ] Test GET /categories endpoint with authentication
  - [ ] Test endpoint returns only enabled categories
  - [ ] Test endpoint requires authentication (401 without JWT)
