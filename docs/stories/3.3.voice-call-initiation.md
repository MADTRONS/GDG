...
- [ ] Update handleVoiceCall in CounselorCard component (AC: 1, 2, 3)
  - [ ] Remove placeholder toast notification
  - [ ] Add loading state (isVoiceLoading)
  - [ ] Make POST request to /api/voice/create-room
  - [ ] Pass counselor category UUID in request body
  - [ ] Handle response data (room_url, user_token, session_id)
  - [ ] Add error handling with try-catch
  
- [ ] Implement navigation to voice session page (AC: 4)
  - [ ] Use Next.js router to navigate to /voice-session
  - [ ] Pass room credentials via router state or query params
...
- [ ] Prevent duplicate requests (AC: 6)
  - [ ] Disable button while isVoiceLoading is true
  - [ ] Add guard clause to prevent multiple simultaneous calls
  - [ ] Show visual feedback (spinner icon replaces microphone)
  
- [ ] Verify authentication requirement (AC: 7)
  - [ ] Check useAuth hook for current user
  - [ ] Redirect to login if not authenticated
  - [ ] Show error message if session expired
  
- [ ] Ensure voice button styling matches design (AC: 8)
  - [ ] Use Mic icon from lucide-react
  - [ ] Button should be primary (filled) variant
  - [ ] Spinner icon during loading state
  - [ ] Maintain accessibility (aria-label, keyboard support)
  
- [ ] Write comprehensive tests (AC: 9)
  - [ ] Mock /api/voice/create-room API call
